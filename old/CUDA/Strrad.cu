...
__global__ void Radix(char * d_in, char * d_out, int * d_scan,  int * d_scatter, int * d_past, int size, int * d_suffix)
{
    int idx = threadIdx.x ;
    int total;
...
int main() {
    ...
    cudaMalloc((void**) &d_suffix, Array_int);
    ...
    // launch the kernel
    Radix<<< 1, size >>>(d_in, d_out, d_scan, d_scatter, d_past, size, d_suffix);

    // copy back the GPU array to the CPU array
    cudaMemcpy(h_out, d_out, Array_char, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_suffix, d_suffix, Array_int, cudaMemcpyDeviceToHost);
